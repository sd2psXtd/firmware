...
const char SUPERBLOCK_VERSION[] = "1.2.0.0\0\0\0\0";
...
{
    register int32_t xor0, xor1, xor2, xor3, xor4;
    uint8_t eccbuf[12];
...
static int Card_Changed(void)
{
    ...
    return mcfat_cardchanged ? sceMcResChangedCard : sceMcResSucceed;
}
...
static int Card_WritePageData(int32_t page, uint8_t *pagebuf, uint8_t *eccbuf)
{
    ...
    mcfat_bdoperations.page_write(&mcfat_cardspecs, page, pagebuf);
    if (mcdi->cardflags & CF_USE_ECC) {
        mcfat_bdoperations.ecc_write(&mcfat_cardspecs, page, eccbuf);
...
static int Card_ReadPageData(int32_t page, uint8_t *pagebuf, uint8_t *eccbuf)
{
    ...
    uint16_t pagesize = read_le_uint16((uint8_t *)&mcdi->pagesize);
    int count = pagesize >> 7;

    mcfat_bdoperations.page_read(&mcfat_cardspecs, page, mcfat_cardspecs.pagesize, pagebuf);

    if (mcdi->cardflags & CF_USE_ECC) {
        mcfat_bdoperations.ecc_read(&mcfat_cardspecs, page, mcfat_cardspecs.pagesize, eccbuf);
    }
...
static int Card_ReplaceBackupBlock(int32_t block)
{
    ...
    for (i = 0; i < 16; i++) {
        if (read_le_uint32((uint8_t *)&mcdi->bad_block_list[i]) == -1)
            break;
    }
...
static int Card_FillBackupBlock1(int32_t block, uint8_t **pagedata, uint8_t *eccdata)
{
    ...
    if ((alloc_offset / clusters_per_block) == block) /* this refuse to take care of a bad rootdir cluster */
        return sceMcResFailReplace;
...
static int Card_WriteCluster(int32_t cluster, int32_t flag)
{
    ...
    for (i = 0; i < 16; i++) { /* check only 16 bad blocks ? */
        if ((int32_t)read_le_uint32((uint8_t *)&mcdi->bad_block_list[i]) < 0)
            break;
        if (read_le_uint32((uint8_t *)&mcdi->bad_block_list[i]) == block)
            return 0;
    }
...
static int Card_FatRSeek(int fd)
{
    ...
    if (entries_to_read < fh->clust_offset)
...
continue_check:
        ...
        } while (++i < read_le_uint32((uint8_t *)&pfse->length));
    }
...
static int Card_SetDirEntryState(int32_t cluster, int32_t fsindex, int32_t flags)
{
    ...
    do {
        ...
        if (mcio_fdhandles[i].cluster != cluster)
            continue;

        if (mcio_fdhandles[i].fsindex == fsindex)
            return sceMcResDeniedPermit;
    ...
    if (fat_index >= 0) {
        if (fat_index < read_le_uint32((uint8_t *)&mcdi->unknown2))
            append_le_uint32((uint8_t *)&mcdi->unknown2, fat_index);
        append_le_uint32((uint8_t *)&mcdi->unknown5, -1);
...
static int Card_CheckBackupBlocks(void)
{
     ...
     */
    for (r1 = 0; r1 < clusters_per_block; r1++) {
...
static int Card_SetDeviceInfo(void)
{
    ...
    while (cluster_cnt < allocatable_clusters_per_card) {
        if (current_allocatable_cluster >= clusters_per_card)
            break;

        if (((current_allocatable_cluster % clusters_per_block) == 0) \
            || (alloc_offset == current_allocatable_cluster)) {
            iscluster_valid = 1;
            for (r=0; r<16; r++) {
...
static int Card_ReportBadBlocks(void)
{
    ...
    for (block = 0, bad_blocks = 0; (block < (clusters_per_card / clusters_per_block)) && (bad_blocks < 16); block++) {
...
{
    register int r, i, j, z, l;
    int32_t pageword_cnt, page, blocks_on_card, erase_byte, err_cnt;
    uint32_t erase_value;
    struct MCDevInfo *mcdi = (struct MCDevInfo *)&mcio_devinfo;
    ...
    for (i = 1; i < blocks_on_card; i++) {
        ...
        if (read_le_uint32((uint8_t *)&mcdi->cardform) > 0) {
            ...
            for (j = 0; j < 16; j++) {
                ...
                if (read_le_uint32((uint8_t *)&mcdi->bad_block_list[j]) == i)
                    goto lbl1;
            }
        }
        else {
                ...
                if ((mcdi->cardflags & CF_USE_ECC) == 0) {
                    for (l = 0; l < pagesize; l++) {
                        if (mcio_pagebuf[l] != erase_byte)
                            err_cnt++;
                        if (err_cnt >= (clusters_per_block << 6)) {
                            j = 16;
                            break;
                        }
                    }
...
static int Card_Format(void)
{
    ...
    if (read_le_uint32((uint8_t *)&mcdi->cardform) == sceMcResNoFormat) {
        for (i = 0; i < 32; i++)
...
static int Card_FileOpen(char *filename, int flags)
{
    ...
    if (r == 0) {
        ...
        if ((flags & sceMcFileAttrWriteable) != 0) {
            ...
            do {
                ...
                if ((fh2->status == 0) \
                    || (fh2->cluster != cacheDir.cluster) || (fh2->fsindex != cacheDir.fsindex))
                    continue;
    ...
    i = -1;
    if (mcio_dircache[2].length == cacheDir.maxent) {
